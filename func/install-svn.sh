#!/bin/bash
#install svn 1.7, 1.8 or 1.9
# see http://blog.wolfogre.com/posts/install-svn-scripts/
...
CHOICE_VERSION=$1
if [[ -z $CHOICE_VERSION ]]; then
  echo -e "1.7\t1.8\t1.9"
  read -p "install which version of svn?" CHOICE_VERSION
fi

if [[ $CHOICE_VERSION != "1.7" && $CHOICE_VERSION != "1.8" && $CHOICE_VERSION != "1.9" ]]; then
  echo "unknown version: $CHOICE_VERSION"
  exit 1
fi

if [[ $CHOICE_VERSION = "1.7" ]]; then
  SVN_VERSION=1.7.22-1
fi
if [[ $CHOICE_VERSION = "1.8" ]]; then
  SVN_VERSION=1.8.19-1
fi
if [[ $CHOICE_VERSION = "1.9" ]]; then
  SVN_VERSION=1.9.7-1
fi
